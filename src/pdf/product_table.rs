...
pub fn draw_product_table(
    ...
    draw_line(&mut ctx.current_ops, COL_1, Mm(190.0), sub_header_y);

    ctx.y = sub_header_y - Mm(5.0);

    draw_products(ctx, invoice, locale, COL_1)?;

    let table_bottom = ctx.y;

    draw_v_line(&mut ctx.current_ops, COL_1, table_top, table_bottom);
    draw_v_line(&mut ctx.current_ops, Mm(100.0), table_top, table_bottom);
    draw_v_line(&mut ctx.current_ops, Mm(130.0), table_top, table_bottom);
    draw_v_line(&mut ctx.current_ops, Mm(160.0), table_top, table_bottom);
    draw_v_line(&mut ctx.current_ops, Mm(190.0), table_top, table_bottom);

    draw_line(&mut ctx.current_ops, COL_1, Mm(190.0), table_bottom);
